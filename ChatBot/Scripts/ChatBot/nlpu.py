...
greeting_regex = re.compile(fr'\b(?i)({"|".join(greetings)})\b')

false = ["false", "no", "nah"]
false_regex = re.compile(fr'\b(?i)({"|".join(false)})\b')

true = ["true", "yes", "yeah", "yh"]
true_regex = re.compile(fr'\b(?i)({"|".join(true)})\b')
...
def get_entities(json):
    ...
    # Answer
    if false_regex.search(str(message)):
        results['answer'] = False
    if true_regex.search(str(message)):
        results['answer'] = True
